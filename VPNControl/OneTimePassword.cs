...
public class OneTimePassword
{
    ...
    {
        byte[] buffer = new byte[this.SecretLength];
    ...
    public long Counter
    {
        get
        {
            ...
            {
                DateTime currTime = (this.TestTime > DateTime.MinValue) ? this.TestTime : DateTime.UtcNow;
                long seconds = (currTime.Ticks - Epoch.Ticks) / 10000000;
    ...
    public int GetCode(int digits)
    {
        ...
        int code = GetCode(counter, digits);
    ...
    private int GetCode(long counter, int digits)
    {
        ...
        byte[] truncatedHash = new byte[] { (byte)(hash[offset + 0] & 0x7F), hash[offset + 1], hash[offset + 2], hash[offset + 3] };
        if (BitConverter.IsLittleEndian) { Array.Reverse(truncatedHash, 0, 4); }
        int number = BitConverter.ToInt32(truncatedHash, 0);
    ...
    {
        int currCode = GetCode(this.Counter, this.Digits);
        int prevCode = GetCode(this.Counter - 1, this.Digits);

        bool isCurrValid = (code == currCode);
        bool isPrevValid = (code == prevCode) && (this.Counter > 0); //don't check previous code if counter is zero; but calculate it anyhow (to keep timing)
        bool isValid = isCurrValid || isPrevValid;
    ...
    private static void FromBase32(string text, byte[] buffer, out int length)
    {
        ...
        { //always assume padding - easier to code than actually checking
            ...
            int bits = Base32Alphabet.IndexOf(char.ToUpperInvariant(ch));
            if (bits < 0) { throw new FormatException("Unknown character '" + ch + "'."); }

            int bitCount1 = (bitPosition < 3) ? 5 : 8 - bitPosition; //how many bits go in current partial byte
            int bitCount2 = 5 - bitCount1; //how many bits are for next byte
    ...
    private static string ToBase32(byte[] bytes, int length, SecretFormatFlags format)
    {
        ...
        bool hasSpacing = (format & SecretFormatFlags.Spacing) == SecretFormatFlags.Spacing;
        bool hasPadding = (format & SecretFormatFlags.Padding) == SecretFormatFlags.Padding;
        bool isUpper = (format & SecretFormatFlags.Uppercase) == SecretFormatFlags.Uppercase;

        int bitLength = (length * 8);
        int textLength = bitLength / 5 + ((bitLength % 5) == 0 ? 0 : 1);
        int totalLength = textLength;

        int padLength = (textLength % 8 == 0) ? 0 : 8 - textLength % 8;
        totalLength += (hasPadding ? padLength : 0);

        int spaceLength = totalLength / 4 + ((totalLength % 4 == 0) ? -1 : 0);
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            {
                int bitsIndex = (bits >> (bitsRemaining - 5)) & 0x1F;
                bitsRemaining -= 5;
        ...
        if (bitsRemaining > 0)
        {
            int bitsIndex = (bits & Base32Bitmask[bitsRemaining]) << (5 - bitsRemaining);
            chars[index] = isUpper ? Base32Alphabet[bitsIndex] : char.ToLowerInvariant(Base32Alphabet[bitsIndex]);
            index++;
        }
